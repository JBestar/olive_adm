...
class KsBet_model extends Model 
{
    protected $table = 'bet_kenoladder';
    ...
    function addBetRound($arrBetData, $objUser, $objConfKs)
    {
        $strRatio = "1.95";
        if($arrBetData['mode'] == 1){
            $strRatio = $objConfKs->game_ratio_1;
        } else if($arrBetData['mode'] == 2){
            $strRatio = $objConfKs->game_ratio_2;
        } else if($arrBetData['mode'] == 3){
            $strRatio = $objConfKs->game_ratio_3;
        } 
    ...
    function getBetSumByMode($arrRoundInfo, $objConfKs){
        ...
        for($i = 0; $i <3; $i ++){
            ...
            $strSql .= " SELECT bet_mb_uid, bet_mode, bet_target, bet_ratio, SUM(bet_money) AS bet_money_sum, mb_game_ks_percent FROM ".$this->table;
     ...
     function getBetSumByDay($arrReqInfo, $objConfKs){
