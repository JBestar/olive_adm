...
class EosBet_Model extends Model
{
    ...
    public function getBetSumByMode($arrRoundInfo, $objConf)
    {
        ...
        for ($i = 0; $i < 4; ++$i ) {
            $iMode = $i + 1;
            // $strSql = ' SELECT SUM(bet_money_sum * mb_game_eo_percent DIV 100) AS bet_money_allsum FROM ( ';
            $strSql = ' SELECT SUM(bet_money_sum) AS bet_money_allsum FROM ( ';
            $strSql .= ' SELECT bet_mb_uid, bet_mode, bet_target, bet_ratio, SUM(bet_money) AS bet_money_sum, mb_game_eo_percent, mb_game_eo2_percent FROM '.$this->table;
        ...
        for ($i = 4; $i < 29; ++$i ) {
            $iMode = $i + 1;
            // $strSql = ' SELECT SUM(bet_money_sum * mb_game_eo2_percent DIV 100) AS bet_money_allsum FROM ( ';
            $strSql = ' SELECT SUM(bet_money_sum) AS bet_money_allsum FROM ( ';
            $strSql .= ' SELECT bet_mb_uid, bet_mode, bet_target, bet_ratio, SUM(bet_money) AS bet_money_sum, mb_game_eo_percent, mb_game_eo2_percent FROM '.$this->table;
            $strSql .= ' JOIN '.$this->mMemberTable.' ON '.$this->mMemberTable.'.mb_uid = '.$this->table.'.bet_mb_uid ';
            $strSql .= " WHERE bet_round_no='".$arrRoundInfo['round_no']."' AND bet_state = '1' ";
